...
namespace Ignis
{
    ...
    static MTL::LoadAction to_metal_load_action(GRILoadAction action)
    {
        switch (action)
        {
            case GRILoadAction::Load:     return MTL::LoadActionLoad;
            case GRILoadAction::Clear:    return MTL::LoadActionClear;
            case GRILoadAction::DontCare: return MTL::LoadActionDontCare;
    ...
    static MTL::StoreAction to_metal_store_action(GRIStoreAction action)
    {
        switch (action)
        {
            case GRIStoreAction::Store:    return MTL::StoreActionStore;
            case GRIStoreAction::DontCare: return MTL::StoreActionDontCare;
    ...
    void MetalStateCache::set_render_pass_info(const GRIRenderPassInfo& info)
    {
        ...
        for (uint32_t i = 0; i < info.get_num_colour_targets(); i++)
        {
            ...
            colour_attachment->setClearColor(MTL::ClearColor::Make(entry.clear_value.r, entry.clear_value.g, entry.clear_value.b, entry.clear_value.a));
        }
