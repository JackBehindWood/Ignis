...
namespace Ignis
{
    ...
    void MetalCommandContext::end_frame()
    {
        ...
        if (m_active_viewport)
        {
            ...
            blit->copyFromTexture(
                m_active_viewport->get_current_backbuffer()->get_texture(),
    ...
    void MetalCommandContext::set_render_targets(uint32_t num_render_targets, const GRIRenderTargetView* render_targets, const GRIDepthRenderTargetView* depth_stencil_target)
    {
        ...
        GRIDepthRenderTargetView depth_target = depth_stencil_target
            ? *depth_stencil_target
            : GRIDepthRenderTargetView(nullptr);
    ...
    {
        m_state_cache.set_index_buffer(
            resource_cast<GRIBuffer>(buffer)->get_buffer(),
    ...
    void MetalCommandContext::draw_indexed_primitives(uint32_t index_count, uint32_t first_index, int32_t vertex_offset)
    {
        ...
        const uint32_t index_stride = (m_state_cache.get_index_type() == MTL::IndexTypeUInt16) ? sizeof(uint16_t) : sizeof(uint32_t);
        ...
        m_render_encoder->get()->drawIndexedPrimitives(
            m_state_cache.get_primitive_type(),
            index_count,
            m_state_cache.get_index_type(),
            m_state_cache.get_index_buffer(),
            byte_offset,
            1,             // instanceCount
