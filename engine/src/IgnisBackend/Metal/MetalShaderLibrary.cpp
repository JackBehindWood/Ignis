                                                           ...
                                                           const String& entry_point)
{
    ...
    dispatch_data_t ddata = dispatch_data_create(data, size,
    ...
    if (!library)
    {
        IG_CORE_ERROR("MetalShaderLibrary: failed to create library from memory (entry '{}'): {}",
                      entry_point, error->localizedDescription()->utf8String());
        return nullptr;
    }
