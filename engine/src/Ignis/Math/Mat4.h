...
namespace Ignis::Math
{
    ...
    struct Mat4
    {
        ...
        constexpr Mat4(T c0r0, T c0r1, T c0r2, T c0r3,
        ...
        constexpr T&       at(int32_t row, int32_t col)       { return m[col * 4 + row]; }
        constexpr const T& at(int32_t row, int32_t col) const { return m[col * 4 + row]; }
    ...
    template<typename T>
    Mat4<T> transposed(const Mat4<T>& mat) { return mat.transposed(); }
