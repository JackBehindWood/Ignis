...
namespace Ignis::Math
{
    ...
    template<typename T>
    constexpr Vec4<T> operator*(T s, const Vec4<T>& v) { return v * s; }

    template<typename T> constexpr T       dot(const Vec4<T>& a, const Vec4<T>& b) { return a.dot(b); }
    template<typename T> T                 length(const Vec4<T>& v) { return v.length(); }
    template<typename T> T                 length_sq(const Vec4<T>& v) { return v.length_sq(); }
    template<typename T> Vec4<T>           normalized(const Vec4<T>& v) { return v.normalized(); }
