...
namespace Ignis
{
    ...
    public:
        ...
        SharedPtr<Material> get_or_create(SharedPtr<RenderShader> vs, SharedPtr<RenderShader> ps,
                                          const String& layout);

        // Reuses cached PSO, attaches caller-supplied params buffer. Result is not cached.
        SharedPtr<Material> create_with_params(SharedPtr<RenderShader> vs, SharedPtr<RenderShader> ps,
        ...
        void set_render_target_format(GRIPixelFormat fmt) { m_rt_format    = fmt; }
        void set_depth_format(GRIPixelFormat fmt)         { m_depth_format = fmt; }
