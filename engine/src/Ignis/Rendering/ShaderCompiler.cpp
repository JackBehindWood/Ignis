...
// ---------------------------------------------------------------------------
    ...
    Vector<ShaderStageOutput> ShaderCompiler::compile(const String &source, const ShaderCompilerOptions &opts)
    {
        ...
        for (uint8_t i = 0; i < opts.count; ++i)
        {
            ...
            const char *entry_point = req.entry_point.empty() ? k_stage_default_entry[(size_t)req.stage] : req.entry_point.c_str();
            const Vector<uint32_t> spv = m_hlsl->compile_to_binary(source, entry_point, req.stage, opts.defines);
            if (spv.empty())
                return {};

            const ShaderReflection refl = Utils::translate_reflection(m_hlsl->reflect(spv.data(), static_cast<uint32_t>(spv.size())));

            const Vector<uint8_t> bin = m_backend->spirv_to_backend_binary(spv.data(), static_cast<uint32_t>(spv.size()), req.stage);
            if (bin.empty())
                return {};
