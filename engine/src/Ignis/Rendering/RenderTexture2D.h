...
namespace Ignis
{
    ...
    public:
        RenderTexture2D(GRITexture2DPtr texture, uint32_t width, uint32_t height, GRIPixelFormat format)
            : m_texture(std::move(texture)), m_width(width), m_height(height), m_format(format)
        {}

        GRITexture2D*  get_texture() const { return m_texture.get(); }
        uint32_t       get_width()   const { return m_width; }
        uint32_t       get_height()  const { return m_height; }
        GRIPixelFormat get_format()  const { return m_format; }
