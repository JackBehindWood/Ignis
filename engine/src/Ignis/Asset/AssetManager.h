...
struct WatchEntry
{
    Path                          source_path;
    AssetID                       id;
    Filesystem::file_time_type    last_mtime = Filesystem::file_time_type::min();
};
...
{
public:
    void watch(const Path& source_path, AssetID id);
    void unwatch(AssetID id);
    void poll_changes(AssetManager& manager);

    void set_poll_interval(uint32_t frames) { m_poll_interval_frames = frames; }

private:
    Vector<WatchEntry> m_entries;
    uint32_t m_poll_interval_frames = 30;
    uint32_t m_frames_since_poll    = 0;
};
...
struct AssetManagerConfig
{
    ...
    uint32_t watcher_poll_interval_frames = 30;
...
public:
    ...
    AssetID import(const Path& source_path, AssetType type,
                   bool cache = false, uint64_t user_data = 0);
    ...
    void set_reload_callback(void (*callback)(AssetID));
...
private:
    ...
    void (*m_on_asset_reloaded)(AssetID)  = nullptr;
