...
static SharedPtr<Asset> parse_mesh_payload(AssetID id, R& r)
{
    ...
    if (!r.good())
    {
        IG_CORE_ERROR("MeshHandler: corrupted payload for asset {0}",
...
bool MeshHandler::compile(const AssetMetadata& metadata)
{
    ...
    if (!w.is_open())
    {
        IG_CORE_ERROR("MeshHandler: could not open output '{}'",
...
SharedPtr<Asset> MeshHandler::load(const AssetMetadata& metadata)
{
    ...
    if (!r.is_open())
    {
        ...
        if (!r.is_open())
        {
            IG_CORE_ERROR("MeshHandler: failed to open compiled asset for '{}'",
                          metadata.compiled_path.string());
            return nullptr;
        }
    }
    ...
    if (!r.good())
    {
        IG_CORE_ERROR("MeshHandler: corrupted binary for '{}'",
                      metadata.compiled_path.string());
        return nullptr;
    }
...
SharedPtr<Asset> MeshHandler::load_from_bytes(const AssetMetadata& metadata,
                                               const Vector<uint8_t>& bytes)
{
    MemBinaryReader r(bytes.data(), bytes.size());
    SharedPtr<Asset> asset = parse_mesh_payload(metadata.ID, r);
    if (!asset)
        IG_CORE_ERROR("MeshHandler: corrupted payload for '{}'",
                      metadata.compiled_path.string());
    return asset;
}
