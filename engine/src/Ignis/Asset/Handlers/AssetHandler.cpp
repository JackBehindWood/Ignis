...
AssetHandler* AssetHandler::get(AssetType type)
{
    ...
    switch (type)
    {
        case AssetType::Shader:
            return &s_shader_handler;
        case AssetType::Mesh:
            return &s_mesh_handler;
        case AssetType::Texture2D:
            return &s_texture_handler;
        case AssetType::Material:
            return &s_material_handler;
        default:
            return nullptr;
    }
}
...
AssetBinaryWriter AssetHandler::open_writer(const AssetMetadata& metadata, const AssetBlobHeader& header,
...
AssetBinaryReader AssetHandler::open_reader(const AssetMetadata& metadata, const AssetBlobHeader& header) const
{
    AssetBinaryReader reader = AssetBinaryReader::open(metadata, header);
    if (!reader.is_open())
    {
        IG_CORE_WARN("AssetHandler: failed to open reader for '{0}'", metadata.compiled_path.string());
    }
    return reader;
}
